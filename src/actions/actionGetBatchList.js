...
export const actionGetBatchList = () => {
  return dispatch => {
    // console.log(JSON.stringify(data))
    dispatch({type: types.GET_BATCH_LIST_START})
    return axios.get(`${GLOBAL.API_HOST}/batchDetails`)
    .then(res => {
      console.log(res);
      if(res.data.status === 200){
        //toast.success(res.data.message);
        dispatch({type: types.GET_BATCH_LIST_SUCCESS, payload: res.data.data})
      }else{
        dispatch({type: types.GET_BATCH_LIST_FAILURE, payload: res.data.statusMessage})
        toast.error(res.data.message);
      }
    })
    .catch(err => {
      dispatch({type: types.GET_BATCH_LIST_FAILURE, payload: err})
    })
  }
}
